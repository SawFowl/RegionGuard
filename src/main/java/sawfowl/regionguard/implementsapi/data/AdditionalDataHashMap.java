...
public class AdditionalDataHashMap<T extends AdditionalData> extends HashMap<String, Map<String, T>> implements AdditionalDataMap<T> {
	...
	private void remove(String plugin, String dataName) {
		if(!containsKey(plugin) || !get(plugin).containsKey(dataName)) return;
		get(plugin).remove(dataName);
		if(get(plugin).isEmpty()) remove(plugin);
	}
