...
public class Blocks extends AbstractCommand {
	...
	public void process(CommandCause cause, Audience audience, Locale locale, boolean isPlayer, Mutable arguments, RawArgumentsMap args) throws CommandException {
		...
		target.sendMessage(getText(target.locale(), LocalesPaths.COMMAND_SETLIMITBLOCKS_SUCCESS_TARGET).replace(new String[] {Placeholders.SIZE, Placeholders.PLAYER}, toSet, sourceName).get());
	}
	...
	public List<RawArgument<?>> getArgs() {
		return Arrays.asList(
			RawArguments.createPlayerArgument(false, false, 0, null, null, null, LocalesPaths.COMMAND_SETLIMITBLOCKS_EXCEPTION_PLAYER_NOT_PRESENT),
			RawArguments.createLongArgument("Value", new ArrayList<Long>(), false, false, 1, null, null, null, null, LocalesPaths.COMMAND_SETLIMITBLOCKS_EXCEPTION_VOLUME_NOT_PRESENT)
		);
	}
