...
public class Members extends AbstractCommand {
	...
	public void process(CommandCause cause, Audience audience, Locale locale, boolean isPlayer, String[] args, Mutable arguments) throws CommandException {
		...
		if(toSet < 0) throw new CommandException(plugin.getLocales().getText(locale, LocalesPaths.COMMAND_SETLIMITMEMBERS_EXCEPTION_LESS_THEN_ZERO));
		plugin.getAPI().setLimitMembers(target, toSet);
		audience.sendMessage(plugin.getLocales().getTextWithReplaced(locale, ReplaceUtil.replaceMap(Arrays.asList(ReplaceUtil.Keys.SIZE, ReplaceUtil.Keys.PLAYER), Arrays.asList(toSet, target.name())), LocalesPaths.COMMAND_SETLIMITMEMBERS_SUCCESS_SOURCE));
	...
	public List<RawArgument<?>> getArgs() {
		return Arrays.asList(
			RawArguments.createPlayerArgument(false, false, 0, LocalesPaths.COMMAND_SETLIMITMEMBERS_EXCEPTION_PLAYER_NOT_PRESENT),
			RawArguments.createLongArgument(new ArrayList<Long>(), false, false, 1, null, LocalesPaths.COMMAND_SETLIMITMEMBERS_EXCEPTION_VOLUME_NOT_PRESENT)
		);
	}
