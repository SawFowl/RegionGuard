...
public class SetBlocksLimitCommand implements PluginRawCommand {
	...
	public CommandResult process(CommandCause cause, Mutable arguments, List<String> args) throws CommandException {
		...
		if(args.size() == 0 || !Sponge.server().player(args.get(0)).isPresent()) throw new CommandException(plugin.getLocales().getText(sourceLocale, LocalesPaths.COMMAND_SETLIMITBLOCKS_EXCEPTION_PLAYER_NOT_PRESENT));
		if(args.size() == 1) throw new CommandException(plugin.getLocales().getText(sourceLocale, LocalesPaths.COMMAND_SETLIMITBLOCKS_EXCEPTION_VOLUME_NOT_PRESENT));
		if(!NumberUtils.isParsable(args.get(1))) throw new CommandException(plugin.getLocales().getText(sourceLocale, LocalesPaths.COMMAND_SETLIMITBLOCKS_EXCEPTION_WRONG_ARGUMENT));
		long toSet = Long.valueOf(args.get(1));
		if(toSet < 0) throw new CommandException(plugin.getLocales().getText(sourceLocale, LocalesPaths.COMMAND_SETLIMITBLOCKS_EXCEPTION_LESS_THEN_ZERO));
		ServerPlayer target = Sponge.server().player(args.get(0)).get();
		plugin.getAPI().setLimitBlocks(target, toSet);
		((Audience) src).sendMessage(plugin.getLocales().getTextWithReplaced(sourceLocale, ReplaceUtil.replaceMap(Arrays.asList(ReplaceUtil.Keys.SIZE, ReplaceUtil.Keys.PLAYER), Arrays.asList(toSet, target.name())), LocalesPaths.COMMAND_SETLIMITBLOCKS_SUCCESS_SOURCE));
	...
	public CommandException usage() throws CommandException {
		throw new CommandException(text("Usage: /rg setlimit blocks [Player] [Volume]"));
	}
